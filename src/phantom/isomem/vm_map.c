...
static volatile int snapper_ready_flag = 0; // Added
...
static void vm_map_snapshot_thread(void)
{
    ...
    while (!snapper_ready_flag)   // Added
        hal_sleep_msec(500);      // Added
    ...
    while(1)
    {
        ...
        if( stop_lazy_pageout_thread )
        {
          ...
          do_snapshot();
            ...
            phantom_shutdown(0); // Added (instead of hal_exit_kernel_thread();)
            // hal_exit_kernel_thread();
        }
